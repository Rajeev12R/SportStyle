    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <Card className="max-w-3xl mx-auto shadow-xl animate-pulse">
            <CardHeader><div className="h-8 w-3/4 bg-muted rounded mb-2"></div><div className="h-4 w-1/2 bg-muted rounded"></div></CardHeader>
    ...
    <div className="container mx-auto px-4 py-12">
      <Card className="max-w-3xl mx-auto shadow-xl">
        ...
        <CardContent className="space-y-8">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <Link href="/checkout/address" passHref>
                <Button variant="outline" size="sm"><Edit className="mr-2 h-4 w-4" />Change</Button>
              </Link>
            </CardHeader>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
               <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-xl">Items in Order ({orderItems.length})</CardTitle>
              </div>
              <Link href="/cart" passHref>
                <Button variant="outline" size="sm"><Edit className="mr-2 h-4 w-4" />Edit Cart</Button>
              </Link>
            </CardHeader>
            <CardContent className="space-y-4">
              {orderItems.map(item => (
                <div key={item.id} className="flex items-center gap-4 py-2 border-b last:border-b-0">
                  <Image src={item.images[0]} alt={item.name} width={60} height={60} className="rounded-md object-cover" data-ai-hint={item.dataAiHint || "product image"} />
          ...
          <Card>
            <CardHeader><CardTitle className="text-xl">Price Details</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between text-muted-foreground"><span>Estimated Taxes</span><span>${taxes.toFixed(2)}</span></div>
              <Separator />
